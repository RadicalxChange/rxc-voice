...
class TransferSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        recipient_autogenerated = recipient_data.get('is_autogenerated', True)
        if recipient_autogenerated is True:
            email_invite(recipient_data.user.email, validated_data.get('sender'), validated_data.get('amount'))
        transfer = Transfer.object.create(
            sender=validated_data.get('sender'),
            recipient=recipient_data,
            amount=validated_data.get('amount'),
            date=validated_data.get('date'),
            status=('A' if recipient_exists else 'P'),
            process=process,
            )
        return transfer
...
class DelegateSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        print("delegate updating") # check for transfers to this ID
